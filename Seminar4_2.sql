...
-- вывести записи где количество авто такой марки больше 2, записи на должны дублироваться
...
FROM AUTO a 
join man m on m.PHONENUM = a.PHONENUM
where m.CITYCODE in (select CITYCODE from city where PEOPLES > 1000000)
**/
/** Вариант 2
SELECT a.MARK, a.COLOR, m.FIRSTNAME, m.LASTNAME 
FROM AUTO a 
...
join man m 
on m.PHONENUM = a.PHONENUM
join (select CITYCODE from city where PEOPLES > 1000000) c 
on m.CITYCODE = c.CITYCODE;
**/
